...
export default function Navbar() {
  ...
  const linkClass = (path) =>
    `inline-block text-base min-[1084px]:text-lg min-[1200px]:text-xl min-[1400px]:text-2xl min-[1920px]:text-3xl font-medium transition-transform duration-200 hover:scale-105 ${location.pathname === path ? "text-[#52b8f4]" : "text-white"
    }`;
  ...
  return (
    ...
    >
      {/* ── Desktop Nav ── */}
      <div className="hidden min-[1084px]:flex justify-center items-center h-28 min-[1400px]:h-32 space-x-2 min-[1200px]:space-x-4 min-[1400px]:space-x-6 min-[1920px]:space-x-10 relative max-w-7xl min-[1920px]:max-w-[1600px] min-[2000px]:max-w-[1800px] mx-auto px-4">

        {/* Left links */}
        {navLinksLeft.map((link) => renderNavLink(link))}

        {/* ── Centred Logo ── */}
        <div className="relative z-[130] -ml-2 min-[1200px]:-ml-4">
          <div className="transform scale-[0.6] min-[1200px]:scale-[0.7] min-[1400px]:scale-[0.8] min-[1920px]:scale-[0.9] flex items-center justify-center">
            <div className="flex items-center justify-center">
              {/* Outer wrapper keeps the logo and the orbit overlay stacked */}
              <div className="relative w-[136px] h-[128px] flex items-center justify-center">
                ...
                {/* Animated orbit — sibling HTML div, not inside SVG */}
      ...
      {/* ── Mobile Nav ── */}
      <div className="min-[1084px]:hidden flex justify-between items-center h-20 px-6 relative z-[110]">
        <Link to="/" className="text-white font-bold text-xl" onClick={closeMobileMenu}>
          Global<span className="text-[#52b8f4]">Web</span>
        </Link>
        
        <button 
          onClick={toggleMobileMenu}
          className="w-10 h-10 flex flex-col justify-center items-center gap-1.5 focus:outline-none z-[120]"
          aria-label="Toggle Menu"
        >
          <motion.span 
            animate={isMobileMenuOpen ? { rotate: 45, y: 8 } : { rotate: 0, y: 0 }}
            className="w-8 h-0.5 bg-white block transition-all" 
          />
          <motion.span 
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            <div className="mt-12 pt-8 border-t border-white/10 text-white/40 text-sm">
              <p>© 2026 Global Web Production</p>
              <p className="mt-2 tracking-widest uppercase text-[10px]">Technology Solutions with Global Impact</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
